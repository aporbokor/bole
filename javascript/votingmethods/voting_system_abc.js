    ...
    for (let i = 0; i < voters.length; i++) {
      voters[i].on_select = function () {
        ...
        for (let j = 0; j < this.voted_for.length; j++) {
          ...
          let arr = new Arrow(cand.color,
            `${this.name}'s ballot arrow`,
            this,
            cand);

          arr.text = int_to_serial_number(j + 1);
          arr.text_label = `${cand.get_name_p().outerHTML}'s place in ${this.get_name_p().outerHTML}'s ballot`;
        }
      }
    }
...
class CondorcetVotingMethod extends RankingVotingMethod {
  ...
  show_outranking_matrix() {
    ...
    let last_choice = voter_res[voter_res.length - 1].get_simple_name_p().outerHTML
    ...
    const preference = voting_sytem.outranking_matrix[voter_res[0].id][voter_res[1].id]
    const preference2 = voting_sytem.outranking_matrix[voter_res[1].id][voter_res[0].id]
  ...
  arrows_between_candidates() {
    ...
    for (const cands of combinations(this.candidates, 2)) {
      ...
      arr = new Arrow(winner.color,
        `${winner.name} beats ${loser.name}`,
        winner,
        loser);

      arr.text = abs(strength);
      arr.text_label = `This many voters prefer ${winner.get_name_p().outerHTML} over ${loser.get_name_p().outerHTML}` +
        ` than ${loser.get_name_p().outerHTML} over ${winner.get_name_p().outerHTML}`;

      arr.start_person_data = `This candidate is prefered over ${loser.get_name_p().outerHTML} by ${this.outranking_matrix[winner.id][loser.id]} voters.`;
      arr.end_person_data = `This candidate is prefered over ${winner.get_name_p().outerHTML} by ${this.outranking_matrix[loser.id][winner.id]} voters.`;
