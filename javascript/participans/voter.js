  ...
  get_extra_to_div() {
    ...
    if (this.supports.length != 0) {
      for (let i = 0; i < this.supports.length; i++) {
        ...
        item.appendChild(this.supports[i].get_name_p(`| supporters: ${this.supports[i].supporters}`));
    ...
    if (this.strategic) {
      ...
      if (seems_win_candidates.length != 0) {
        tactical_p.innerText = 'From the supporter counts of each candidate this tactical voter has concluded that these are the candidates, who are likely to win:';
        ...
        for (let i = 0; i < seems_win_candidates.length; i++) {
          let li = document.createElement('li');
          li.appendChild(seems_win_candidates[i].get_name_p(`| supporters: ${seems_win_candidates[i].supporters}`))
          tactical_ul.appendChild(li)
        }

        tactical_div.appendChild(tactical_ul);
      } else {
        tactical_p.innerText = 'From the supporter counts of each candidate this tactical voter was not able to predict which candidates are likely to win.'
        tactical_div.appendChild(tactical_p);
      }
